...
- Go to the new folder internet-outbound-microhack and initialize the terraform modules and download the azurerm resource provider
...
- Verify that your VNet Peering and Site-to-site VPN are funcitoning as expected: From the "wvd-workstation" VM, access the on-prem Linux box via SSH (IP address: 10.57.2.4).
...
In the Azure portal, find the Route Table associated to the wvd-workstation's subnet (wvd-spoke-rt) and add a default route to send all internet-bound traffic to on-prem, via the site-2-site IPSec tunnel:
...
The on-prem proxy performs TLS inspection by terminating on itself the TLS connection initiated by your browser and setting up a new one between itself and the server. As the proxy does not have access to the server's private key, it dynamically generates a new certificate for the server's FQDN and signs it with its own certificate. This is reflected in the certification path shown by the browser.

![image](images/cert-path.png)

The proxy's own certifcate is self-signed, and therefore not trusted by your browser. However, as a Contoso user, you are willing to trust it, which you can do by installing it in your certificate store. 
...
- Browse to https://ipinfo.io again. Now you can successfully the site. Verify hat the public IP you're using to access the internet is actually the proxy's public IP:
...
- Browse to any other sites and confirm that your connection is blocked by the proxy 
...
- All connections initaited by the wvd-workstation are routed to Contoso's on-prem datacenter
...
Would this configuration work if, instead of a standalone VM, the wvd-workstation was a real Azure WVD session host? Review [this article](https://docs.microsoft.com/en-us/azure/virtual-desktop/safe-url-list) before answering the question. 
...
In the Azure Portal, deploy a new Azure Firewall instance in the hub-vnet. An "AzureFirewallSubnet" has been already created for you. 
